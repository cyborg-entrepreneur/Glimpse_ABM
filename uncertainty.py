...
class KnightianUncertaintyEnvironment:
    ...
    def measure_uncertainty_state(
        ...
        actor_level_no_ai = float(np.clip(stable_sigmoid(raw_actor_no_ai - 0.35), 0.01, 0.99))
        ...
        novelty_level = float(np.clip(stable_sigmoid(2.1 * (raw_novelty - 0.05)), 0.05, 0.95))
        novelty_diagnostics = {
        ...
        agentic_level_no_ai = float(np.clip(
            0.25 + 0.35 * novelty_level + 0.25 * combo_pressure + 0.25 * scarcity_signal + 0.1 * disruption_avg,
            0.0,
            1.0
        ))
